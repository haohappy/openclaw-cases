...
在 Apple Music 中播放音乐，然后：

```bash
./music.sh              # 默认：音频响应模式
./music.sh --club       # 夜店模式：高饱和度，随音乐节拍快速变化
./music.sh --work       # 工作模式：柔和暖白，无动画
...
| 模式 | 命令 | 特点 |
|------|------|------|
| 默认 | `./music.sh` | 根据流派生成渐变色板，音量驱动亮度，强节拍时旋转 |
| 夜店 | `./music.sh --club` | 高饱和互补色，每帧旋转，音量大幅驱动明暗变化 |
| 工作 | `./music.sh --work` | 低饱和暖白，无动画，10 秒更新一次 |
| BPM | `./music.sh --bpm` | 按歌曲 BPM 定时旋转，不需要音频捕获 |

---

## 工作原理

1. **音乐检测**：通过 osascript 查询 Apple Music 当前曲目、艺术家、流派
2. **颜色映射**：流派 → 基础色调（rock→红橙、jazz→蓝紫、electronic→紫粉、pop→粉红 等），曲名 MD5 哈希 → 同流派内的色彩变化
3. **渐变生成**：3 个锚点色在所有 zone 间线性插值
4. **音频响应**：ffmpeg 从 BlackHole 捕获系统音频 → sox 分析 RMS 音量 → 驱动亮度和旋转
5. **自适应**：自动检测灯带 zone 数量（支持 9-zone 和 48-zone 等不同型号）
