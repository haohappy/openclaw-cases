...
| 夜店 | `./music.sh --club` | 纯色色块 + 强烈闪烁 + 白光频闪（详见下方说明） |
| 工作 | `./music.sh --work` | 暖白光（与 `nanoleaf warm` 一致），无动画，10 秒更新一次 |
| BPM | `./music.sh --bpm` | 按歌曲 BPM 定时旋转（夜店每拍，默认每 2 拍），不需要音频捕获 |

### 夜店模式详细说明

夜店模式模拟真实夜店灯光效果，与默认模式有本质区别：

**色彩：纯色色块，非渐变**
- 灯带分为 4 段，每段填充一种纯色（如红、蓝、黄、紫），颜色之间无过渡
- 内置 6 套高对比配色方案，每首歌/每次换色随机选择：
  - 红 蓝 黄 紫 / 橙 绿 粉 黄 / 红 青 黄 粉 / 橙 蓝 绿 红 / 红 黄 蓝 粉 / 黄 红 蓝 紫
- 每套配色都保证包含暖色（红、橙、黄），确保视觉冲击力

**明暗：强烈闪烁**
- 音量 < 15 时灯带完全熄灭（模拟夜店灯灭瞬间）
- 音量上升时亮度快速拉起到全亮，制造闪烁效果
- 每帧旋转色块位置，节拍时额外跳跃多个位置

**白光频闪**
- 检测到强节拍（音量 > 60）时，全部 zone 瞬间切换为纯白（255,255,255）
- 闪烁一帧后立即恢复正常色板，模拟夜店频闪灯

**纯音频驱动**
- 无需 Apple Music，任何音频源（mpv、浏览器等）均可驱动
- 约每 20 秒自动更换一套配色方案
